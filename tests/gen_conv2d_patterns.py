...
def conv2d_reference(inp, kernel, bias,
    ...
    out = np.zeros((out_height, out_width, out_channels), dtype=np.int8)
...
def write_inout(path, cfg, inp, kernel, bias,
                out_multiplier, out_shift, golden):
...
def make_multiplier_shift(scale_f32, n_channels):
    ...
    NNLib hard constraint: shift must be in [-31, 31].
    When the natural shift falls outside this range, we fold the excess bits
    into the multiplier (clamped to int32 range) so the shift stays valid.
    This keeps the fixture runnable on HiFi5 NNLib while preserving the
    intent of the test (the golden is always recomputed with the adjusted
    multiplier/shift pair, so CModel vs NNLib comparison remains meaningful).
    """
    import math
    ...
    for _ in range(n_channels):
        ...
        e = math.floor(math.log2(abs(scale_f32)))
        m = scale_f32 / (2 ** e)          # 0.5 <= m < 1
        q31 = round(m * (2**31))

        # Guard q31 boundaries (float rounding near powers-of-2)
        if q31 >= 2**31:
            q31 = 2**31 - 1
        if q31 < 1:
            q31 = 1
        ...
        # Clamp shift to NNLib [-31, 31] range.
        # If shift < SHIFT_MIN (very small scale -> large right-shift),
        # absorb the excess into the multiplier by right-shifting it.
        if shift < SHIFT_MIN:
            excess = SHIFT_MIN - shift      # positive extra bits
            q31 = q31 >> excess
            if q31 < 1:
                q31 = 1
            shift = SHIFT_MIN
        elif shift > SHIFT_MAX:
            excess = shift - SHIFT_MAX
            q31 = min(q31 << excess, 2**31 - 1)
            shift = SHIFT_MAX
...
def make_pattern(pid, desc, purpose,
    ...
    inp    = inp_override    if inp_override    is not None else rand_int8((IH, IW, IC))
    ...
    write_inout(fname, cfg, inp, kernel, bias, mult, shift, golden)
    print(f"  [{pid}] {fname}  "
...
if __name__ == "__main__":
    ...
    if want("p07"):
        make_pattern("p07", "accumulator_near_overflow",
            ...
            inp_override    = np.full((4,4,8),  127, dtype=np.int8),
    ...
    if want("p08"):
        make_pattern("p08", "all_zero_input",
            ...
            pad_h=1, pad_w=1,
            inp_override  = np.zeros((6,6,4), dtype=np.int8),
    ...
    # TFLite filter format: [OC=16, KH=3, KW=3, IC=16]
    # Moderate spatial dims, general 3x3 kernel, no padding (valid conv).
