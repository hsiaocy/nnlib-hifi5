...
static int64_t safe_mul(int32_t val, int shift)
{
    return (int64_t)val * (1LL << shift);
}
                                                 ...
                                                 int     check_flag)
{
    ...
    if (exponent <= 0) {                /* exponent < 0 → right shift */
        ...
        if (check_flag == 1)
            printf("dividend >> exponent is %lld\n", (long long)result);

        int32_t threshold = remainder_mask >> 1;
        if (check_flag == 1)
            printf("threshold = remainder_mask >> 1; is %d\n", threshold);

        if (result < 0) {
            threshold++;
            if (check_flag == 1)
                printf("final threshold is %d\n", threshold);
        }
        ...
        if (check_flag == 1)
            printf("The dividend * 2^exponent(exponent>0) result is: %lld\n",
                   (long long)result);
    }
                                                    ...
                                                    int check_flag)
{
    ...
    if (check_flag == 1) {
        printf("m1 is %d\n",              m1);
        printf("m2 is %d\n",              m2);
        printf("mult.word.low is %d\n",   mult.word.low);
        printf("mult.word.high is %d\n",  mult.word.high);
        printf("before mult.long_long is %lld\n", (long long)mult.long_long);
    }

    mult.long_long = mult.long_long + (int64_t)m1 * m2;

    if (check_flag == 1) {
        int64_t test = (int64_t)m1 * (int64_t)m2;  /* Q4 fix: cast before multiply */
        printf("test is %lld\n",              (long long)test);
        printf("after mult.long_long is %lld\n", (long long)mult.long_long);
    }

    result = (int32_t)(mult.long_long >> 31);

    if (check_flag == 1)
        printf("multi result(m1*m2 >> 31) is %d\n", result);
...
static inline int64_t nn_requantize(int32_t val, int32_t multiplier,
                                     int32_t shift, int check_flag)
{
    if (check_flag == 1) {
        printf("the acc val is %d\n",   val);
        printf("the mult is  %d\n",     multiplier);
        printf("the shift is %d\n",     shift);
    }
    return nn_divide_by_power_of_two(
               nn_doubling_high_mult_no_sat(val, multiplier, check_flag),
               shift, check_flag);
}
                                     ...
                                     int32_t shift)
{
    return nn_divide_by_power_of_two(
               nn_doubling_high_mult_no_sat(
                   val * (1 << LEFT_SHIFT(shift)), multiplier, 0),
               RIGHT_SHIFT(shift), 0);
}
#endif /* shift_change */
...
void cmodel_conv2d_per_chan_sym8sxasym8s(
    ...
    int out_stride)
{
    /* Q5 fix: heap allocation instead of VLA to avoid stack overflow
     * on large output tensors (e.g. 56x56x64 needs ~800 KB on stack). */
    int total_out = output_height * output_width * output_channels;
    int32_t *partialsum = (int32_t *)malloc(total_out * sizeof(int32_t));
    if (!partialsum) {
        fprintf(stderr, "[cmodel] malloc failed for partialsum (%d elems)\n",
                total_out);
        return;
    }

    FILE *fp_partialsum = fopen("SW_KW5_partialsum_data.txt", "w");
  ...
  #ifdef shift_change
                int check_flag = 0;
                partialsum[oh * out_stride + ow * output_channels + oc] = acc;
                if (fp_partialsum)
                    fprintf(fp_partialsum, "%x\n",
                            partialsum[oh * out_stride + ow * output_channels + oc]);
    ...
    if (fp_partialsum)
        fclose(fp_partialsum);

    free(partialsum);   /* Q5 fix: release heap allocation */
}
