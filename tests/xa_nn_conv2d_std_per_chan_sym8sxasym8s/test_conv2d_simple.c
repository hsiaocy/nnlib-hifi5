...
typedef struct {
    ...
    WORD32 *partial_sum_golden;
    WORD32  partial_sum_size;
} pattern_t;
...
static int load_pattern(const char *buf, pattern_t *pat)
{
    ...
    pat->partial_sum_golden = (WORD32 *)malloc(max_out * sizeof(WORD32));
...
static int compare_partial_sum(const WORD32 *ps_nnlib, const WORD32 *ps_golden, WORD32 size)
{
    WORD32 n_miss = 0;
    for (WORD32 i = 0; i < size; i++) {
        if (ps_nnlib[i] != ps_golden[i]) {
            if (n_miss < 10)
                printf("  PS Mismatch[%d]: got %d  expected %d\n",
                       i, (int)ps_nnlib[i], (int)ps_golden[i]);
            n_miss++;
        }
    }
    if (n_miss)
...
int main(void)
{
    ...
    /* 3. Allocate output + scratch + partial_sum capture buffer */
    ...
    WORD32 *ps_buf = (WORD32 *)malloc(out_size * sizeof(WORD32));
    if (!ps_buf) { printf("ERROR: ps_buf malloc failed\n"); free(output); return 1; }
    memset(ps_buf, 0, out_size * sizeof(WORD32));
    ...
    /* 4. Call HiFi5 Conv2D */
    printf("Calling xa_nn_conv2d_std_v2_per_chan_sym8sxasym8s()...\n\n");
    ...
    WORD32 ret = xa_nn_conv2d_std_v2_per_chan_sym8sxasym8s(
        output,
        ...
        pat.out_activation_min, pat.out_activation_max,
        NULL,        /* p_dma_cfg */
        ps_buf);     /* p_partial_sum: capture before requantize */
